...
import {
  resetSessionId,
  setInputMode as persistInputMode,
...
export const Hud = () => {
  ...
  const percentage =
    topIndividualDps > 0 ? (youRdps / topIndividualDps) * 100 : 0;
        ...
        <div className="mb-2 rounded-lg border border-cyan-400/20 bg-cyan-500/8 px-2.5 py-2 no-drag">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="text-right text-[10px] text-cyan-100/65">
              ...
              <div>
                {debugData
                  ? `${Math.round(debugData.elapsedMs / 1000)}s`
                  : "--"}
              </div>
            </div>
          </div>
          ...
          {debugData?.currentPlayer ? (
            <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-[11px] tabular-nums">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="font-semibold text-white">
                  {debugData.currentPlayer.name}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-white/55">JOB</span>
                <span className="font-semibold text-white">
                  {formatJobId(debugData.currentPlayer.jobId)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-white/55">RAW</span>
                <span className="font-semibold text-white">
                  {formatInt(debugData.currentPlayer.totalDamage)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-white/55">RECENT</span>
                <span className="font-semibold text-white">
                  {formatInt(debugData.currentPlayer.recentDamage)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-white/55">RECV</span>
                <span className="font-semibold text-rose-300">
                  {formatInt(debugData.currentPlayer.receivedBuffContribution)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-white/55">GRANT</span>
                <span className="font-semibold text-emerald-300">
                  {formatInt(debugData.currentPlayer.grantedBuffContribution)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-white/55">rDPS</span>
                <span className="font-semibold text-cyan-200">
                  {formatInt(debugData.currentPlayer.onlineRdps)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
            ...
            <div className="mt-2">
              ...
              <div className="mt-1 flex flex-wrap gap-1.5">
                {debugData.currentPlayer.activeBuffs.slice(0, 8).map((buff) => (
                  ...
                  >
                    {buff.buffName ||
                      `0x${buff.buffId.toString(16).toUpperCase()}`}
                  </div>
                ))}
              </div>
            </div>
